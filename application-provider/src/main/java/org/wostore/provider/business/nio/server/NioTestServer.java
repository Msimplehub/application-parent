package org.wostore.provider.business.nio.server;
...
public class NioTestServer extends Thread {
	...
	private void doWriteMessage(SelectionKey key) throws IOException, UnsupportedEncodingException {
		...
		System.out.println("is writable" + sc.socket().getRemoteSocketAddress());
		ByteBuffer buffer = ByteBuffer.wrap(("how are you?" + sc.socket().getRemoteSocketAddress()).getBytes("UTF-8"));
